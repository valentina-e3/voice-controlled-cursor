...
def save_results(username, mode, elapsed_time):
    with open("mode_testing_results.csv", "a") as f:
        ...
        dt = datetime.now()
        writer.writerow([username, mode, elapsed_time, dt])
...
def show_input_fields():
    ...
    username_entry_label.place(
        x=root.winfo_screenwidth() // 2 - 100, y=root.winfo_screenheight() // 2 - 180
    )
    username_entry.place(
        x=root.winfo_screenwidth() // 2 - 100, y=root.winfo_screenheight() // 2 - 150
    )
    mode_combobox_label.place(
        x=root.winfo_screenwidth() // 2 - 100, y=root.winfo_screenheight() // 2 - 120
    )
    mode_combobox.place(
        x=root.winfo_screenwidth() // 2 - 100, y=root.winfo_screenheight() // 2 - 90
    )
    ok_button.place(
        x=root.winfo_screenwidth() // 2 + 55, y=root.winfo_screenheight() // 2 - 40
    )
...
def handle_button_click(button_index):
    ...
    if button_index == BUTTON_ORDER[current_button]:
        buttons[button_index].config(
            bg="green"
        )  # Set button color to green for correct order
        current_button += 1
        ...
        buttons[button_index].config(
            bg="red"
        )  # Set button color to red for wrong order
...
root = tk.Tk(screenName=APP_NAME, baseName=APP_NAME, className=APP_NAME)
...
start_button = tk.Button(
    ...
    fg="white",
    bg="red",
    command=handle_start_button_click,
)
